...
def _get_model_shortname(model_name):
    ...
    engine_shortname = engine_shortname[:-len("-Turbo")] if engine_shortname.endswith("-Turbo") else engine_shortname
    engine_shortname = engine_shortname[len("Meta-"):] if engine_shortname.startswith("Meta-") else engine_shortname
...
class ModelCallHandler:
    def __init__(self, model_name, model_access_method, lora_model_path=None):
        ...
        assert model_access_method in [
            'vllm-python', 'vllm-python-lora', 'vllm-api', 
            'openai-azure-api', 'openai-api', 
            'huggingface-4bit', 'huggingface'
        ]
        if model_access_method == 'vllm-python':
            self.load_vllm_python()
        if model_access_method == 'vllm-python-lora':
            self.load_vllm_python_with_lora_request(lora_model_path)  # model_name = base model ; lora_model_path = fine-tuned model with LORA
        ...
        assert model_access_method != 'huggingface', f"model_access_method={model_access_method} not implemented."
    ...
    def is_vllm_api_available():
        ...
        try:
            ...
            assert output.status_code == 404, "I do not know a context in which there would be another status code output"
            return True
    ...
    def load_vllm_python(self):
        ...
        self.vllm_llm = LLM(model=self.model_name, tensor_parallel_size=torch.cuda.device_count())
    ...
    def call_openai_api_retry_loop(self, message_list, max_tokens=10, top_p=1.0, temperature=1.0, **kwargs):
        try:
            ...
            print(e)
            time.sleep(0.50)
            return self.call_openai_api_retry_loop(message_list, max_tokens, top_p, temperature, **kwargs)
    ...
    def call_model(
            self, prompt, max_tokens=50, top_p=1.0, temperature=1.0, system_prompt=None, **kwargs):
        ...
        assert sum([1 for m in message_list if m["role"] == "system"]) < 2, f"There should not be two system messages: {str(message_list)}"

        if self.model_access_method.startswith('huggingface'):
            return self.call_huggingface_local_model(message_list, max_tokens, top_p, temperature)

        if self.model_access_method.startswith('vllm-python'):
            return self.call_vllm_python_model(message_list, top_p, temperature, **kwargs)

        if self.model_access_method.startswith('openai') or self.model_access_method == 'vllm-api':
            return self.call_openai_api_retry_loop(message_list, max_tokens, top_p, temperature, **kwargs)

        assert False, f"Unsupported model access method {self.model_access_method}"

    def verify_using_llm_as_judge(self, parsed_fields, llm_judge_prompt, llm_judge_prompt_answers):
        ...
        return {
            'is_equivalent': len(answers) == len(llm_judge_prompt_answers) and all(a == b for a, b in zip(answers, llm_judge_prompt_answers)),
...
"""
    ...
    def verify_answer_to_question(self, answer, model_response, num_tries=1):
        if isinstance(answer, str):
            ...
            parsed_fields = {
                ...
                'desiderata': "\n".join([f"* {e}: (yes/no)" for e in correct_descriptions + incorrect_descriptions])
            }
            llm_judge_prompt_answers = ['yes'] * len(correct_descriptions) + ['no'] * len(incorrect_descriptions)
            ...
            for idx_try in range(num_tries):
                if idx_try > 0:
                    ...
                    llm_judge_prompt_answers = [llm_judge_prompt_answers[i] for i in shuffled_indices]
                    parsed_fields['desiderata'] = [parsed_fields['desiderata'][i] for i in shuffled_indices]

                output = self.model_call_handler.verify_using_llm_as_judge(parsed_fields, ModelEvaluator.CHECK_ANSWERS_PROMPT, llm_judge_prompt_answers)
    ...
    def evaluate_question(self, story, question, expected_answer, num_tries=1):
        model_response, tokens_used = self.model_call_handler.call_model(
            self.FULL_PROMPT.format(story=story, question=question), max_tokens=40, top_p=1.0, temperature=0.0
        )
        model_response = model_response.lower()

        response = {
            'story': story, 'model_response': model_response, 'question': question, 'answer': expected_answer,
            'is_correct': self.verify_answer_to_question(expected_answer, model_response, num_tries=num_tries)
        }
        return response, tokens_used

    def evaluate_story(self, belief_tracker, story=None, nth=1, num_tries=1, keep_only_interesting_qs=False, heuristic_always_return_correct=False):
        all_tokens_used = 0
        responses = []

        story = " ".join(belief_tracker.story_script) if story is None else story
        first_order_questions = QuestionGenerator(belief_tracker).main(nth_reasoning_order=nth)
        for question, answer, _, metadata in first_order_questions:
            ...
            if heuristic_always_return_correct:
                ...
                response, tokens_used = self.evaluate_question(story, question, answer, num_tries=1)
    ...
    def evaluate_file(self, df, output_filename_evaluated):
        ...
        for i, row in df.iterrows():
            oupu, _ =  self.evaluate_question(row['story'], row['question'], row['expected_answer'], num_tries=1)
