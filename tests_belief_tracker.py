...
def test1():
    function_list = [
        ...
        lambda o: o.private_communication("David", "Mark", topic=('project prototype', FullBeliefTracker.CONTAINER_LOCATION, 'wooden crate', True)),
    ]
...
def test_ice_cream_van_story():
    ...
    function_list = [
        lambda o: o.location_declaration(A, room_name),  # A = friend of the one getting ice cream
        ...
        lambda o: o.private_communication(C, A, topic=('truck', FullBeliefTracker.ROOM_LOCATION, 'church', True)),
        lambda o: o.private_communication(C, A, topic=(C, FullBeliefTracker.ROOM_LOCATION, 'church', True)),
        lambda o: o.private_communication(C, B, topic=('truck', FullBeliefTracker.ROOM_LOCATION, 'church', True)),
        lambda o: o.private_communication(C, B, topic=(C, FullBeliefTracker.ROOM_LOCATION, 'church', True)),
    ]
    return function_list
...
def test_tomi_story():
    ...
    function_list = [
        lambda o: o.enter_room(people_name_list[0], room_name),
        lambda o: o.enter_room(people_name_list[1], room_name),
        lambda o: o.move_object_container(people_name_list[1], obj='ball', container='box'),
        lambda o: o.leave_room(people_name_list[1], room_name),
        lambda o: o.move_object_container(people_name_list[0], obj='ball', container='basket'),
        lambda o: o.enter_room(people_name_list[1], room_name),  # person_1 should not magically know the basket is in the box
    ]
    return function_list
...
def test_apple():
    ...
    function_list = [
        ...
        lambda o: o.update_object_state(
            people_name_list[1], 'apple', 'poisoned', '<is_edible>', f"{people_name_list[1]} poisoned the apple.", False
        ),
        lambda o: o.leave_room(people_name_list[1], room_name),
        lambda o: o.update_object_state(
            people_name_list[0], 'apple', 'peeled', '<is_peeled>', f"{people_name_list[0]} peeled the apple.", True
        ),
        lambda o: o.update_object_state(
            people_name_list[0], 'apple', 'covered in chocolate', '<is_covered_in_chocolate>', f"{people_name_list[0]} covered the {'apple'} in chocolate.", True
        ),
        lambda o: o.enter_room(people_name_list[2], room_name),
...
def test_asymmetric():
    ...
    function_list = [
        ...
        lambda o: o.move_object_container(
            'Benjamin', object_name, 'file cabinet', people_peeking=['John'], people_distracted=['Sara']
        ),
    ]
    return function_list
...
def _check_that_questions_are_not_repeated(belief_tracker):
    ...
    first_order_questions = [question for question, answer, entities, metadata in question_generator.main(1)]
    second_order_questions = [question for question, answer, entities, metadata in question_generator.main(2)]
    assert len(first_order_questions) == len(set(first_order_questions)), "There are repeated first-order questions"
    assert len(second_order_questions) == len(set(second_order_questions)), "There are repeated second-order questions"



if __name__ == '__main__':
    for function_list in [
            test_asymmetric(), test_debug_interesting_q(), test_apple(), test_tomi_story(), 
            test_ice_cream_van_story(), test_fantom(), test3(), test2(), test1()
        ]:
        ...
        for nth_reasoning_order in [1, 2]:
            for question, answer, entities, metadata in QuestionGenerator(bt).main(nth_reasoning_order=nth_reasoning_order):
                ...
                cot = chain_of_thought_generator.main(
                    bt.story_script_tuple_representation, question, entities, answer, nth_reasoning_order=nth_reasoning_order)
                #if cot:
                #    print('')
