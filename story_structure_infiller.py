...
"""
    ...
    JUDGING_STORY_FREQUENT_ERRORS_PROMPT_EXPECTED_ANSWERS = ['yes'] * len(JUDGING_STORY_FREQUENT_ERRORS_PROMPT_QUESTIONS)

    def __init__(self, model_call_handler, **kwargs):
        self.model_call_handler = model_call_handler
        self.num_tries_llm_judge_structure = kwargs.get('num_tries_llm_judge_structure', 2)
    ...
    def _turn_all_textual_beliefs_into_enumeration(self, prev_world_state_list, prev_first_order_beliefs_list, prev_second_order_beliefs_list, question_suffix='', shuffle_statements=False):
        ...
        prev_world_state = '\n'.join([f"* {e}{question_suffix}" for e in prev_world_state_list])
        prev_first_order_beliefs = '\n'.join([f"* {e}{question_suffix}" for e in prev_first_order_beliefs_list])
        prev_second_order_beliefs = '\n'.join([f"* {e}{question_suffix}" for e in prev_second_order_beliefs_list])
    ...
    def _parse_llm_as_a_judge_output(cur_world_state, model_response):
        ...
        cur_world_state_lines = sorted([ws.rstrip('(yes/no/unsure)').lower().replace('?', '') for ws in cur_world_state_lines])
        model_response = [line.lower().replace('?', '') for line in model_response.strip().split('\n') if line.startswith('*')]
        model_response = [mr_line for mr_line in model_response if any(mr_line.startswith(ws_line) for ws_line in cur_world_state_lines)]
        ...
        for clean_ws, model_response_elem in zip(cur_world_state_lines, model_response):
            ...
            if not model_response_elem.startswith(clean_ws) or model_output not in ['yes', 'no', 'unsure']:
                return [], False
            responses.append((clean_ws, model_output))
        return responses, True

    def check_story_structure(self, prev_belief_tracker, cur_belief_tracker, shuffle_statements, new_action_text, skip_second_order_qs=False):
        prev_world_state_list, prev_first_order_beliefs_list, prev_second_order_beliefs_list = prev_belief_tracker.get_all_beliefs_in_textual_form()
        prev_world_state, prev_first_order_beliefs, prev_second_order_beliefs = self._turn_all_textual_beliefs_into_enumeration(
            prev_world_state_list, prev_first_order_beliefs_list, prev_second_order_beliefs_list)

        cur_world_state_list, cur_first_order_beliefs_list, cur_second_order_beliefs_list = cur_belief_tracker.get_all_beliefs_in_textual_form()
        cur_world_state, cur_first_order_beliefs, cur_second_order_beliefs = self._turn_all_textual_beliefs_into_enumeration(
            cur_world_state_list, cur_first_order_beliefs_list, cur_second_order_beliefs_list, question_suffix='?: (yes/no/unsure)', shuffle_statements=shuffle_statements)

        num_calls = 0
        to_analyze = [
            (prev_world_state, cur_world_state, 'world_state'),
            (prev_world_state + '\n' + prev_first_order_beliefs, cur_first_order_beliefs, 'first_order'),
            (prev_world_state + '\n' + prev_first_order_beliefs + '\n' + prev_second_order_beliefs, cur_second_order_beliefs, 'second_order')
        ]
        ...
        for prev, cur, ws_type in to_analyze:
            ...
            filled_prompt = self.JUDGING_STORY_STRUCTURE_PROMPT.format(prev_world_state=prev, story_action=new_action_text, new_world_state=cur)
            partial_chat, model_response = [{"role": "user", "content": filled_prompt}], None
            for retry_idx in range(self.num_tries_llm_judge_structure):
                ...
                if retry_idx and self.CLARIFYING_ANSWERS_PROMPT_TITLE in model_response:
                    model_response = model_response.split(self.CLARIFYING_ANSWERS_PROMPT_TITLE)[-1]
                responses, llm_as_a_judge_succeeded = self._parse_llm_as_a_judge_output(cur, model_response)
            ...
            if not all_checked_ok:
                if not llm_as_a_judge_succeeded:
                    ...
                    return False, [(q, r) for q, r in responses if r != 'yes'], num_calls
        return True, [], num_calls

    def check_story_structure_with_shuffling_retries(self, cur_belief_tracker, next_belief_tracker, new_action_text, skip_second_order_qs=False):
        ...
        for retry_idx in range(self.num_tries_llm_judge_structure):
            ...
            output, possible_issues, num_calls = self.check_story_structure(
                cur_belief_tracker, next_belief_tracker, shuffle_statements=(retry_idx > 0), new_action_text=new_action_text,
    ...
    def check_story_surface_form_with_retries(self, accum_story, next_story_step):
        ...
        for retry_idx in range(self.num_tries_llm_judge_surface):
            questions_ordered_indices = list(range(len(self.JUDGING_STORY_SURFACE_FORM_PROMPT_QUESTIONS)))
            if retry_idx > 0:
                random.shuffle(questions_ordered_indices)
            qs = '\n'.join([self.JUDGING_STORY_SURFACE_FORM_PROMPT_QUESTIONS[i] for i in questions_ordered_indices])
            expected_answers = [self.JUDGING_STORY_SURFACE_FORM_PROMPT_EXPECTED_ANSWERS[i] for i in questions_ordered_indices]

            model_response, tokens_used = self.model_call_handler.call_model(
                self.JUDGING_STORY_SURFACE_FORM_PROMPT.format(accum_story=accum_story, story_action=next_story_step, itemized_questions=qs),
                max_tokens=800,
                top_p=1.0,
                temperature=0.0,
            )
    ...
    def adhoc_story_checks_with_shuffling_retries(self, story_action_raw, story_action_infilled):
        """
        ...
        for retry_idx in range(self.num_tries_llm_judge_adhoc):
            questions_ordered_indices = list(range(len(self.JUDGING_STORY_FREQUENT_ERRORS_PROMPT_QUESTIONS)))
            if retry_idx > 0:
                random.shuffle(questions_ordered_indices)
            qs = '\n'.join([self.JUDGING_STORY_FREQUENT_ERRORS_PROMPT_QUESTIONS[i] for i in questions_ordered_indices])
            expected_answers = [self.JUDGING_STORY_FREQUENT_ERRORS_PROMPT_EXPECTED_ANSWERS[i] for i in questions_ordered_indices]
            model_response, tokens_used = self.model_call_handler.call_model(
                self.JUDGING_STORY_FREQUENT_ERRORS_PROMPT.format(story_action_raw=story_action_raw, story_action_infilled=story_action_infilled, itemized_questions=qs),
                max_tokens=800, top_p=1.0, temperature=0.0,
            )
            num_calls_total += 1
    ...
    def main(self, cur_belief_tracker, next_belief_tracker, next_story_context, next_story_step, next_story_step_raw):
        """
        ...
        """
        accum_infilled_story_without_new_step = next_story_context[:-len(next_story_step)]
        is_story_surface_valid, possible_issues_surface_list, num_calls_llm_judge = self.check_story_surface_form_with_retries(
            accum_infilled_story_without_new_step, next_story_step,
        )

        is_story_structure_valid, possible_issues_structure_list, num_calls_llm_judge_2 = self.check_story_structure_with_shuffling_retries(
            cur_belief_tracker, next_belief_tracker, next_story_step, skip_second_order_qs=True
        )  # skipping second order qs since models are quite bad on them

        is_adhoc_checks_valid, possible_issues_adhoc_checks_list, num_calls_llm_judge_3 = self.adhoc_story_checks_with_shuffling_retries(
            next_story_step_raw, next_story_step,
        )

        tmp_for_log_infilling = {
            'story_context': "Empty" if next_story_context is None else next_story_context,
        ...
        next_story_step_accepted = (is_story_structure_valid and is_story_surface_valid and is_adhoc_checks_valid)
...
STORY CONTINUATION: <fill>"""
    ...
    def __init__(self, model_call_handler, **kwargs):
        self.model_call_handler = model_call_handler
        self.evaluator = ModelAnswerEvaluator(model_call_handler)  # this is just to evaluate the generated story here and have all answers in the logs
        self.quality_verifier = StoryInfillingQualityVerifier(model_call_handler, **kwargs)

        self.SAMPLE_CHARACTERS_GOALS = kwargs.get('sample_characters_goals', True)  # include character goals to story prompt
        self.SAMPLE_INITIAL_STORY_CONTEXT = kwargs.get('sample_initial_story_context', True)  # sample context before talking about actions
        self.num_tries_completions = kwargs.get('num_tries_completions', 5)
        self.sample_all_next_step_completions_simultaneously = kwargs.get('sample_all_next_step_completions_simultaneously', False)  # this adds diversity
        self.infilling_grouping_type = kwargs.get('infilling_grouping_type', 'dst1')  # this groups actions for extra naturalness
    ...
    def _sample_characters_goals(self, story_script, characters_description):
        ...
        full_prompt = self.PROMPT_CHARACTER_GOALS.format(story_script=story_script, characters_description=characters_description)
        characters_goals, tokens_used = self.model_call_handler.call_model(
            self.PROMPT_CHARACTER_GOALS.format(story_script=story_script, characters_description=characters_description),
            max_tokens=300,
            top_p=1.0,
            temperature=1.0,
        ...
        characters_goals = characters_goals[len('CHARACTERS GOALS:'):] if characters_goals.startswith('CHARACTERS GOALS:') else characters_goals
    ...
    def _sample_initial_infilling_context(self, story_script, characters_description):
        if not self.SAMPLE_INITIAL_STORY_CONTEXT:
            return None
        full_prompt = self.PROMPT_INITIAL_STORY_CONTEXT.format(story_script=story_script, characters_description=characters_description)
        story_context, tokens_used = self.model_call_handler.call_model(
            self.PROMPT_INITIAL_STORY_CONTEXT.format(story_script=story_script, characters_description=characters_description),
            max_tokens=300,
            top_p=1.0,
            temperature=1.0,
            repetition_penalty=1.0,
    ...
    def _select_prompt_variations(self, relevant_personas, cur_story_context):
        ...
        if 'fantom_like_data' in self.experiment_variations:
            ...
            prompt_story_length = self.PROMPT_STORY_INFILLING_VERSIONS['story_length'][random.randint(0, len(self.PROMPT_STORY_INFILLING_VERSIONS['story_length']) - 1)]
            prompt_infilling_text_type = self.PROMPT_STORY_INFILLING_VERSIONS['infilling_text_type'][random.randint(0, len(self.PROMPT_STORY_INFILLING_VERSIONS['infilling_text_type']) - 1)]
        return prompt_story_length, prompt_infilling_text_type, people_with_personas, story_context

    def _single_sample_next_story_steps_for_infilling(self, relevant_personas, new_information, cur_story_context, characters_goals=None):
        prompt_story_length, prompt_infilling_text_type, people_with_personas, story_context = self._select_prompt_variations(relevant_personas, cur_story_context)
    ...
    def _simultaneously_sample_next_story_steps_for_infilling(self, relevant_personas, new_information, cur_story_context, characters_goals=None):
        ...
        for num_try_idx in range(self.num_tries_completions):
            ...
            next_story_step_list = [e[len('STORY CONTINUATION:'):].strip() if e.startswith('STORY CONTINUATION:') else e for e in next_story_step_list if e]
            ...
            next_story_step_list = [e for e in next_story_step_list if '\n' not in e] # do not want a sentence infilling that has multiple paragraphs
    ...
    def _group_actions_for_infilling(self, story_script, story_script_tuple_representation):
        """
        ...
        if self.infilling_grouping_type == 'dst1':
            ...
            for i in range(1, len(story_script_tuple_representation)):
                ...
                if len(prev) == len(cur) and prev[0] == cur[0] and sum(1 for e1, e2 in zip(prev, cur) if e1 == e2) >= len(prev) - 1:
                    groups[-1].append(story_script[i])
                    ctr += 1
                    groups_indices[-1].append(ctr)
                elif 'peeking_distracted' in cur[0]:  # this means that it is a modifier of the previous action and should be added
                    groups[-1].append(story_script[i])
                else:
            ...
            assert not any('peeking_distracted' in cur[0] for cur in story_script_tuple_representation), "Did not fix this for asymmetry. Currently not in use."
    ...
    def main(self, belief_tracker, function_list, use_custom_names_dict):
        ...
        characters_description_enumeration = "\n".join([f"* {e}" for e in use_custom_names_dict['people_with_personas']])

        characters_goals = self._sample_characters_goals(story_script, characters_description_enumeration)
        story_context = self._sample_initial_infilling_context(story_script, characters_description_enumeration)

        grouped_story_script, grouped_story_script_indices = self._group_actions_for_infilling(
        ...
        df_list = []
        for new_information, new_information_indices in zip(grouped_story_script, grouped_story_script_indices):
            new_action_function = [function_list[i] for i in new_information_indices]
            relevant_personas = [
                p for p_n, p in zip(use_custom_names_dict['people_names'], use_custom_names_dict['people_with_personas'])
                if p_n in new_information]

            if self.sample_all_next_step_completions_simultaneously:
                next_story_step_list, tokens_used, log_info_regardless_of_retry_idx = self._simultaneously_sample_next_story_steps_for_infilling(
                    relevant_personas, new_information, story_context, characters_goals,
                )
            ...
            for retry_idx in range(self.num_tries_completions):
                if self.sample_all_next_step_completions_simultaneously:
                    ...
                    next_story_step, tokens_used, log_info_regardless_of_retry_idx = self._single_sample_next_story_steps_for_infilling(
                        relevant_personas, new_information, story_context, characters_goals
                    )

                next_story_context = next_story_step if story_context is None else story_context + " " + next_story_step

                next_belief_tracker = copy.deepcopy(cur_belief_tracker)
                for fn in new_action_function:
                    output_flag = fn(next_belief_tracker)
                assert " ".join(next_belief_tracker.story_script).endswith(new_information), f"{next_belief_tracker.story_script} does not end with {new_information}"

                next_story_step_accepted, tmp_for_log_infilling_clean = self.quality_verifier.main(
                    cur_belief_tracker, next_belief_tracker, next_story_context, next_story_step, new_information,
                )
        ...
        if next_story_step_accepted:
            ...
            assert belief_tracker.story_script == cur_belief_tracker.story_script, f"{belief_tracker.story_script} != {cur_belief_tracker.story_script}"
        else:
        ...
        responses_raw_script_evaluations, tokens_used = self.evaluator.evaluate_story(belief_tracker, story=None, nth=1)
        responses_infilled_script_evaluations, tokens_used = self.evaluator.evaluate_story(belief_tracker, story=story_context, nth=1)

        return df_list, (belief_tracker, function_list), story_context, next_story_step_accepted, responses_raw_script_evaluations, responses_infilled_script_evaluations, self.log_infilling
...
class FullStoryStructureInfiller:
    def __init__(self, model_call_handler, **kwargs):
        ...
        self.SAMPLE_INITIAL_STORY_CONTEXT = kwargs.get('sample_initial_story_context', True)
        self.num_tries_completions = kwargs.get('num_tries_completions', 5)
        self.sample_all_next_step_completions_simultaneously = kwargs.get('sample_all_next_step_completions_simultaneously', True)
    ...
    def main(self, logs_directory, NUM_STORIES, NUM_PARALLEL, parallel_idx=None, only_verified_cases=True):
        ...
        filenames_to_solve = [
            ...
            if f.startswith('search') and f.endswith('.pkl') and \
                self.model_call_handler.model_shortname in f and \
                'dill' in f and \
                'weight-goal4' in f and \
                f'_n_{NUM_STORIES}_' in f
        ]
        if 'fantom_like_data' in self.experiment_variations:
            filenames_to_solve = [f for f in filenames_to_solve if '+fantom+' in f or '_fantom-' in f or '_all_' in f]

        random.seed(1)
        random.shuffle(filenames_to_solve)

        for i, filename in enumerate(filenames_to_solve):
            if parallel_idx is not None and i % NUM_PARALLEL != parallel_idx % NUM_PARALLEL:  # DIY parallelization hahaha
                continue
            print('f', filename)
            ...
            prefix_filename_to_write, full_filename_to_write_pkl = self._build_output_filename(filename, len(all_logs))
            ...
            for log_idx, log in enumerate(all_logs):
                df_list, (belief_tracker, function_list), story_context, next_story_step_accepted, responses_raw_script_evaluations, responses_infilled_script_evaluations, self.log_infilling = self.infiller.main(log['belief_tracker'], log['function_list'], log['use_custom_names_dict'])
                pd.DataFrame(df_list).to_csv(os.path.join(base_directory, f'{prefix_filename_to_write}_idx-{log_idx}.csv'))
                big_pickle_data_list.append([df_list, (belief_tracker, function_list), story_context, next_story_step_accepted, responses_raw_script_evaluations, responses_infilled_script_evaluations, self.log_infilling])
            with open(os.path.join(base_directory, full_filename_to_write_pkl), 'wb') as f:
...
if __name__ == '__main__':
    ...
    if args.generate_fantom_like_data:
        full_infiller = FullStoryStructureInfiller(model_call_handler, sample_all_next_step_completions_simultaneously=True, experiment_variations=['fantom_like_data'])
    else:
        full_infiller = FullStoryStructureInfiller(model_call_handler, sample_all_next_step_completions_simultaneously=True)
    full_infiller.main(logs_directory='logs_debug_dec4_fixed_restrictions', NUM_STORIES=args.num_stories_total, NUM_PARALLEL=NUM_PARALLEL, parallel_idx=args.i)    
