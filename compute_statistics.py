...
def _parse_filename(filename):
    ...
    if match:
        return {
            ...
            "param=a_star_neighbor_priority": (
                match.group(15) if match.group(15) else "random"
            ),
        }
    else:
        return None
...
def dump_all_infilled_stories(
    model_call_handler, required_strings, banned_strings, base_dir, output_filename
):
    import numpy as np
    import git
    ...
    for filename in os.listdir(base_dir):
        if not filename.endswith(".pkl"):
            continue
        print()
        if any(e in filename for e in ["_p_5_", "_p_6_", "_m_5_", "_m_6_"]):
            continue
        ...
        for test_idx, (
            ...
            # if isinstance(responses_raw_script, str):
            #    responses_raw_script_evaluations, tokens_used = model_call_handler.evaluate_story(belief_tracker, story=responses_raw_script, nth=1)
            #    responses_infilled_script_evaluations, tokens_used = model_call_handler.evaluate_story(belief_tracker, story=responses_infilled_script, nth=1)
            # else:
            #    responses_raw_script_evaluations = responses_raw_script
            #    responses_infilled_script_evaluations = responses_infilled_script
...
def generate_table_1(df, NUM_STORIES_CONST, BUDGET):
    ...
    # print(set(stats_by_story_type['param=story_type'].tolist()))
    stats_by_story_type["param=story_type"] = pd.Categorical(
        stats_by_story_type["param=story_type"], DESIRED_ORDER
    )
    ...
    ##### print(stats_by_story_type)

    df_pivoted = stats_by_story_type.pivot(
        index="param=story_type",
        columns=["is_q_interesting", "is_asymmetric"],
        values="Accuracy",
    )
    df_pivoted = df_pivoted.reset_index()
    # print(acc_name, max_nodes, sent)
    # print(df_pivoted.to_latex())
...
def summarize_all_cross_models_evaluations(NUM_STORIES_CONST, BUDGET, SAMPLE_SIZE):
    print("ModelName1", "ModelName2", "Accuracy", "WrongRatio")
    ...
    for max_sentences in [10, 15]:
        for num_people, num_moves, num_rooms in itertools.product(
            ...
            for _ in range(n_total):
                ...
                function_list, belief_tracker, action_type_counter, _ = (
...
def compare_astar_vs_baseline(
    ...
    x = comparison.sample(n=162 // 2, random_state=42)  # sample half the settings
    differences = sorted(x["accDiff"].tolist())
    bins = 8
    plt.rc("font", size=18)
    plt.figure()
...
if __name__ == "__main__":
    ...
    for model_name, search_or_baseline, only_verified_cases in itertools.product(
        ...
        output_filename = f"analyses/raw_scripts_dec2_questions_{model_shortname}_{search_or_baseline}_verified_{only_verified_cases}_n_{NUM_STORIES_CONST}_budget_{BUDGET}.csv"
        if os.path.exists(output_filename):
            ...
            print("regex_match_files", regex_match_files)
    ...
    infilled_filename = (
        f"analyses/infilled_dec2_all_{model_shortname}_n_{NUM_STORIES_CONST}.csv"
    )
    if os.path.exists(infilled_filename):
        ...
        df_infilled = dump_all_infilled_stories(
            ...
            ["_fanlike", "_v2"],
            "infilled_with_llm_judge_w_goals_and_initial_debug_dec2_fixed_restrictions",
            infilled_filename,
        )
    ...
    if args.human_eval_doc_generator:
        ...
        generate_human_evaluation_file(
            df,
            "human_evals/human_eval_dec2_raw_1.csv",
            MAX_QUESTIONS_TO_DUMP_PER_STORY,
            MAX_STORIES_TO_DUMP,
            "raw_story",
        )
        generate_human_evaluation_file(
            df_infilled,
            "human_evals/human_eval_dec2_infilled_1.csv",
            MAX_QUESTIONS_TO_DUMP_PER_STORY,
            MAX_STORIES_TO_DUMP,
            "infilled_story",
        )
    ...
    if args.evaluate_cross_model_generations:
        ...
        evaluate_model_on_data_generated_by_other_models(
            df_dict, model_call_handler, SAMPLE_SIZE=250
        )
        summarize_all_cross_models_evaluations(
            NUM_STORIES_CONST, BUDGET, SAMPLE_SIZE=250
        )
        exit(0)
    ...
    generate_table_1(
        df_dict["meta-llama/Meta-Llama-3.1-70B-Instruct"]["search"][False],
        NUM_STORIES_CONST,
        BUDGET,
    )
    generate_table_1(
        df_dict["mistralai/Mixtral-8x7B-Instruct-v0.1"]["search"][False],
        NUM_STORIES_CONST,
        BUDGET,
    )
    exit(0)
